...
withTempRoot("linux-install", (root) => {
  ...
  assert(result.installedVersion === "0.2.15", "linux install should name host version");
...
withTempRoot("linux-release-download", (root) => {
  ...
  const result = runJson(["install", "--release-tag", "v0.2.15"], env);

  assert(result.ok === true, "linux release install should report ok");
  assert(result.installedVersion === "0.2.15", "linux release install should name host version");
  assert(
...
withTempRoot("install", (root) => {
  ...
  assert(result.installedVersion === "0.2.15", "macOS install should name host version");
  assert(
  ...
  const versionDir = join(installRoot, "native-hosts/0.2.15");
  ...
  const metadata = readMetadata(installRoot, "0.2.15");
  assert(metadata.hostVersion === "0.2.15", "metadata should name host version");
...
withTempRoot("macos-wrapper", (root) => {
  ...
  assert(
    result.updaterPath === join(env.HOVER_TRANS_PORT_INSTALL_ROOT, "native-hosts/0.2.15/install.sh"),
...
withTempRoot("macos-legacy-standalone-payload", (root) => {
  ...
  assert(result.installedVersion === "0.2.15", "standalone legacy macOS payload should install host version");
  assert(
    readFileSync(result.helperPath, "utf8") === helperBody,
...
withTempRoot("macos-legacy-pipe", (root) => {
  ...
  assert(result.installedVersion === "0.2.15", "pipe-style legacy macOS installer should install host version");
  assert(
    installerTempEntries(installerTmp).length === 0,
...
withTempRoot("persisted-updater-download", (root) => {
  ...
  assert(result.installedVersion === "0.2.15", "persisted updater should install target version");

  const installedHelper = readFileSync(
    join(installRoot, "native-hosts/0.2.15/hover-trans-port-helper"),
    "utf8"
  );
  assert(
...
withTempRoot("status-uninstall", (root) => {
  ...
  assert(status.includes("installed native host 0.2.15"), "status should report installed version");
