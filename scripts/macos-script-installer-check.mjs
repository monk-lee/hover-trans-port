...
withTempRoot("install", (root) => {
  ...
  assert(output.includes("installed native host 0.2.8"), "install output should name host version");

  const installRoot = env.HOVER_TRANS_PORT_INSTALL_ROOT;
  const versionDir = join(installRoot, "native-hosts/0.2.8");
  ...
  const metadata = readMetadata(installRoot, "0.2.8");
  assert(metadata.hostVersion === "0.2.8", "metadata should name host version");
...
withTempRoot("persisted-updater-download", (root) => {
  ...
  assert(result.installedVersion === "0.2.8", "persisted updater should install target version");

  const installedHelper = readFileSync(
    join(installRoot, "native-hosts/0.2.8/hover-trans-port-helper"),
    "utf8"
  );
  assert(
...
withTempRoot("status-uninstall", (root) => {
  ...
  assert(status.includes("installed native host 0.2.8"), "status should report installed version");
